document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 3. Speed Optimization Simulator Demo
    // ==========================================
    const btnSpeed = document.getElementById('btn-run-speed-audit');
    const gaugeRing = document.getElementById('gauge-ring-circle');
    ...
    if (btnSpeed) {
        btnSpeed.addEventListener('click', () => {
            ...
            btnSpeed.innerHTML = `
                <svg class="ai-ring" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><circle cx="12" cy="12" r="10"></circle></svg>
                <span>Analyzing Asset Bundles...</span>
            `;
            gaugeRing.className = 'gauge-ring running';
            gaugeValue.className = 'gauge-value running';
            gaugeValue.innerText = '..';
            gaugeStatus.className = 'gauge-label running';
            gaugeStatus.innerText = 'Testing Mobile Speed';

            // Step 2: Caching & Deferring Simulation
            setTimeout(() => {
                btnSpeed.innerHTML = `
                    <svg class="ai-ring" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><circle cx="12" cy="12" r="10"></circle></svg>
                    <span>Minifying Scripts & CSS...</span>
                `;
                gaugeValue.innerText = '74';
            }, 800);

            // Step 3: Deferring Scripts
            setTimeout(() => {
                btnSpeed.innerHTML = `
                    <svg class="ai-ring" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><circle cx="12" cy="12" r="10"></circle></svg>
                    <span>Configuring Caching Headers...</span>
                `;
                gaugeValue.innerText = '89';
            }, 1600);

            // Step 4: Final optimization finish
            setTimeout(() => {
                btnSpeed.innerHTML = `
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                    <span>Audit Optimized!</span>
                `;
                gaugeRing.className = 'gauge-ring passing';
                gaugeValue.className = 'gauge-value';
                gaugeValue.innerText = '99';
                gaugeStatus.className = 'gauge-label passing';
                gaugeStatus.innerText = '99/100 Mobile Score';
            }, 2400);
        });
    }

    // ==========================================
    // 4. Previu Live Feedback Pin Mode Simulator
    // ==========================================
    const btnFeedback = document.getElementById('btn-toggle-feedback');
    ...
    if (btnFeedback) {
        btnFeedback.addEventListener('click', (e) => {
            e.stopPropagation(); // Avoid triggering pin placement immediately
            feedbackModeActive = !feedbackModeActive;

            if (feedbackModeActive) {
                // Activate Mode
                btnFeedback.classList.add('active');
                btnFeedback.style.backgroundColor = '#0066cc';
                btnFeedback.style.color = '#ffffff';
                btnFeedback.style.borderColor = '#0066cc';
                document.getElementById('feedback-btn-text').innerText = 'Disable Feedback Mode';
                feedbackInstruction.classList.remove('hide');
                
                // Add page listeners
                document.addEventListener('click', placeFeedbackPin);
            } else {
                // Deactivate Mode
                btnFeedback.classList.remove('active');
                btnFeedback.style.backgroundColor = '';
                btnFeedback.style.color = '';
                btnFeedback.style.borderColor = '';
                document.getElementById('feedback-btn-text').innerText = 'Enable Feedback Mode';
                feedbackInstruction.classList.add('hide');
                
                // Remove page listeners
                document.removeEventListener('click', placeFeedbackPin);
            }
        });
    }

    function placeFeedbackPin(e) {
        // Prevent placing pins on input boxes, buttons, or nav elements
        if (
        ...
        pin.innerText = pinCounter;

        // 2. Create Comment Popover
        const popover = document.createElement('div');
        popover.className = 'comment-popover';
        
        const input = document.createElement('input');
        input.type = 'text';
        input.placeholder = 'Type feedback & press Enter...';

        popover.appendChild(input);
        pin.appendChild(popover);
        document.body.appendChild(pin);

        // Auto-focus input
        setTimeout(() => input.focus(), 50);

        // Keep popover open on input click
        input.addEventListener('click', (ev) => ev.stopPropagation());

        // Press Enter key to lock comment in
        input.addEventListener('keydown', (ev) => {
            if (ev.key === 'Enter') {
                ev.stopPropagation();
                if (input.value.trim() !== '') {
                    // Turn input into plain text comment
                    const commentSpan = document.createElement('span');
                    commentSpan.style.fontSize = '9px';
                    commentSpan.style.color = '#1c1917';
                    commentSpan.style.display = 'block';
                    commentSpan.style.marginTop = '4px';
                    commentSpan.style.fontWeight = '500';
                    commentSpan.innerText = `"${input.value}"`;
                    
                    popover.innerHTML = '';
                    popover.appendChild(commentSpan);
                } else {
                    // Remove if empty
                    pin.remove();
                    pinCounter--;
        ...
        document.addEventListener('click', function closePopover(event) {
            if (!pin.contains(event.target)) {
                if (input.value && input.value.trim() === '') {
                    pin.remove();
                    pinCounter--;
                } else if (input.parentNode) {
                    // If they just clicked away without entering text, remove the pin
                    pin.remove();
                    pinCounter--;
                }
